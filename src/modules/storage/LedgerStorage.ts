...
import {
    ...
    Utils, Endian, Signature, PublicKey, DataPayload
} from 'boa-sdk-ts';
...
export class LedgerStorage extends Storages
{
            ...
            return new Promise<void>((resolve, reject) =>
            {
                storage.run(
                    `INSERT INTO tx_inputs
                        (block_height, tx_index, in_index, tx_hash, utxo, signature, unlock_age)
                    VALUES
                        (?, ?, ?, ?, ?, ?, ?)`,
                    [
                        height.toString(),
            ...
            return new Promise<number>((resolve, reject) =>
            {
                storage.run(
                    `INSERT INTO tx_input_pool
                        (tx_hash, input_index, utxo, signature, unlock_age)
                    VALUES
                        (?, ?, ?, ?, ?)`,
                    [
                        hash.toBinary(Endian.Little),
    ...
    public getTxInputs (height: Height, tx_index: number): Promise<any[]>
    {
        let sql =
        `SELECT
            block_height, tx_index, in_index, utxo, signature, unlock_age
        FROM
            tx_inputs
    ...
    public getTransactionPending (tx_hash: Hash): Promise<Transaction | null>
    {
        return new Promise<Transaction | null>(async (resolve, reject) =>
        {
            try {
                ...
                if (rows.length > 0)
                {
                    let input_rows = await this.query("SELECT tx_hash, utxo, signature FROM tx_input_pool WHERE tx_hash = ? ORDER BY input_index;", [hash]);
